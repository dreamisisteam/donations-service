...
def _sync_members_registry():
    ...
    users_id_by_address_mapping = dict(models.Member.objects.values_list('address', 'user__id'))
    ...
    actual_existing_members_addresses = existing_members_set - deactivated_members_addresses
    ...
    get_user_model().objects.filter(id__in=[
...
def _sync_members_needs():
    members_queryset = models.Member.objects.select_related('user') \
        .only('address', 'needs', 'charges', 'user__is_active').active()

    members_to_update_objs: list[models.Member] = []
    for member_obj in members_queryset.iterator(chunk_size=100):
        needs_info, charges = gateways.contracts_gateway.get_member_needs_state(member_obj.address)
    ...
    models.Member.objects.bulk_update(members_to_update_objs, fields=['needs', 'charges'], batch_size=100)
