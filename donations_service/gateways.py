...
def prepare_address(func):
    """Convert str address to ChecksumAddress"""
...
class DonationContractsGateway:
    ...
    def __init__(self):
        self.exchanger = _load_contract(
            settings.EXCHANGER_CONTRACT_PATH, settings.CONTRACT_ADDRESS
        )
    ...
    def members(self) -> list[str]:
        ...
        cache.set(
            "donation_members", members_registry, settings.CONTRACT_VIEWS_DATA_CACHE
        )
        return members_registry

    def _get_member_needs_names(self, address: str) -> list[str]:
        """
        Get all names of donation program members needs
    ...
    def _get_member_need_total(self, address: str, name: str) -> int:
        """
        Get total sum of donation need of member
    ...
    def _get_member_charges_balance(self, address: str) -> int:
        """
        Get all charges balance of donation program member

        :param address: address of needy
        :return: charges balance
        """
        charges_total_in_wei = self.token.functions.donateBalance(address).call()
        return charges_total_in_wei

    @prepare_address
    def get_member_needs_state(
        self, address: str
    ) -> tuple[ListNeedsInfoType, int, int]:
        """
        Get member needs state

        :param address: address of member
        :return: [{"name": "guitar", "total": 100}, {"name": "course", "total": 30], 100, 130
        """
        total = 0
        ...
        for need_name in self._get_member_needs_names(address):
            need_total = self._get_member_need_total(address, need_name)
        ...
        return member_need_array, charges, total
