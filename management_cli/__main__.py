...
ACCOUNTS_MAPPING_PATH = env.str("ACCOUNTS_MAPPING_PATH", default="local.json")
...
def _send_signed_contract_transaction(w3, contract_method, private_key):
    ...
    transaction = contract_method.build_transaction({"nonce": nonce, "from": address})
...
def register_member(
    w3, token_contract, moderator_private_key, account_address  # internals
):
...
def register_donate_need(
    w3, token_contract, account_private_key, need_name, total  # internals
):
...
def delete_donate_need(w3, token_contract, account_private_key, need_name):  # internals
...
def transfer(
    w3, token_contract, account_private_key, receiver_address, value  # internals
):
...
def donate(w3, token_contract, account_private_key, value):  # internals
...
def donate_all(w3, token_contract, account_private_key, value):  # internals
...
def buy(w3, contract_address, sender_address, sender_private_key, value):  # internals
...
arg_parser.add_argument(
    "action", help=("Specifies action of module\n" "Options: {_actions_mapping.keys()}")
)
arg_parser.add_argument(
    "-f",
    "--accounts-file",
    dest="file",
    default=ACCOUNTS_MAPPING_PATH,
    help=("Specifies accounts file\nType: str\n" "Default: local.json"),
)
arg_parser.add_argument(
    "-s",
    "--sender",
    ...
    default=0,
    type=int,
    help=("Specifies sender index of operation\nType: int\n" "Default: 0"),
)
arg_parser.add_argument(
    "-r",
    "--receiver",
    ...
    default=None,
    type=int,
    help=("Specifies receiver index of operation\nType: int\n" "Default: None"),
)
arg_parser.add_argument(
    "-v",
    "--value",
    default=None,
    help=("Specifies value for operation\n" "Default: None"),
)
...
def main():
    ...
    print(
        "Initial balance: ",
        token_contract.functions.balanceOf(addresses[args.sender]).call(),
    )
    ...
    match args.action:
        ...
        case "register_donate_need":
            ...
            register_donate_need(
                w3, token_contract, private_key, values[0], int(values[1])
            )
